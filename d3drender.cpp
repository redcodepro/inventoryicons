...
{
	strcpy_s( m_szFontName, szFontName );
...
HRESULT CD3DFont::Initialize ( IDirect3DDevice9 *pD3Ddev )
{
	...
	RECT	all = { 0, 0, m_texWidth, m_texWidth };
...
HRESULT CD3DFont::Print( const char *text, D3DCOLOR color, float x, float y )
{
	...
	for ( size_t cpos = 0; cpos < len; cpos++ )
	{
		...
		*pVertex++ = Init2DVertex( xp - 0.5f, yp - 0.5f, color, tx1, ty1 );			//topleft
		*pVertex++ = Init2DVertex( xp + w - 0.5f, yp - 0.5f, color, tx2, ty1 );		//topright
		*pVertex++ = Init2DVertex( xp - 0.5f, yp + h - 0.5f, color, tx1, ty2 );		//bottomleft
		*pVertex++ = Init2DVertex( xp + w - 0.5f, yp - 0.5f, color, tx2, ty1 );		//topright
		*pVertex++ = Init2DVertex( xp + w - 0.5f, yp + h - 0.5f, color, tx2, ty2 );	//bottomright
		*pVertex++ = Init2DVertex( xp - 0.5f, yp + h - 0.5f, color, tx1, ty2 );		//bottomleft
		if ( m_dwCreateFlags & FCR_BORDER )
			w -= 2.0f;
